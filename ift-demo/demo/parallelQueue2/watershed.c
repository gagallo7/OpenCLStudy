...
// This describes the format of the image data.
    ...
    ncolsBuffer = clCreateBuffer (
            contexto,
            CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
            sizeof ( cl_int ),
            &(img->ncols),
            &errNum);
    checkErr(errNum, "clCreateBuffer(cols)");

    nrowsBuffer = clCreateBuffer (
            contexto,
            CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
            sizeof ( cl_int ),
            &(img->nrows),
            &errNum);
    checkErr(errNum, "clCreateBuffer(nrows)");
    ...
    errNum |= clSetKernelArg(kernel0, 0, sizeof(cl_mem), &IvalBuffer);
    errNum |= clSetKernelArg(kernel0, 1, sizeof(cl_mem), &imgtbrowBuffer);
    errNum |= clSetKernelArg(kernel0, 2, sizeof(cl_mem), &Abuffer);
    errNum |= clSetKernelArg(kernel0, 3, sizeof(cl_mem), &Adxbuffer);
    errNum |= clSetKernelArg(kernel0, 4, sizeof(cl_mem), &Adybuffer);
    errNum |= clSetKernelArg(kernel0, 5, sizeof(cl_mem), &SEMbuffer);
    errNum |= clSetKernelArg(kernel0, 6, sizeof(cl_mem), &extraBuffer);
    errNum |= clSetKernelArg(kernel0, 7, sizeof(cl_mem), &cacheBuffer);
    errNum |= clSetKernelArg(kernel0, 8, sizeof(cl_mem), &nrowsBuffer);
    errNum |= clSetKernelArg(kernel0, 9, sizeof(cl_mem), &ncolsBuffer);
    /*
    ...
    */
    ...
    errNum = clSetKernelArg(kernel, 0, sizeof(cl_mem), &IvalBuffer);
    errNum |= clSetKernelArg(kernel, 1, sizeof(cl_mem), &Adxbuffer);
    errNum |= clSetKernelArg(kernel, 2, sizeof(cl_mem), &Adybuffer);
    errNum |= clSetKernelArg(kernel, 3, sizeof(cl_mem), &Mbuffer);
    errNum |= clSetKernelArg(kernel, 4, sizeof(cl_mem), &costBuffer);
    errNum |= clSetKernelArg(kernel, 5, sizeof(cl_mem), &predBuffer);
    errNum |= clSetKernelArg(kernel, 6, sizeof(cl_mem), &imgValBuffer);
    //errNum |= clSetKernelArg(kernel, 4, sizeof(cl_mem), &CPredbuffer);
    ...
    */
    ...
    errNum |= clSetKernelArg(kernel3, 6, localWorkSize [0] * sizeof(cl_int), NULL);
    /*
    errNum |= clSetKernelArg(kernel2, 11, sizeof(cl_mem), &input_image);
    errNum |= clSetKernelArg(kernel2, 12, sizeof(cl_sampler), &sampler);
    */
    ...
    while( Cmin < Cmax ) {
        ...
        */
        ...
        /*
        errNum = clReleaseEvent ( releituraFeita );
        checkErr(errNum, "Releasing event");
        */
    ...
    errNum = clReleaseEvent ( evento0 );
    errNum = clReleaseEvent ( evento1 );
