...
Image *Watershed(Image *img, Set *Obj, Set *Bkg, Image *imgOrig)
{
  ...
  A     = Circular(1.5);
    ...
    cl_int* gmin = (cl_int *) malloc ( (N >> 8) * sizeof ( cl_int ) );
    /*
...
// This describes the format of the image data.
    ...
    gminBuffer = clCreateBuffer (
            contexto,
            CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
            ( N >> 8 ) * sizeof ( cl_int ),
            gmin,
            &errNum);
    checkErr(errNum, "clCreateBuffer(gmin)");
    ...
    */
    ...
    errNum = clSetKernelArg(kernel2, 0, sizeof(cl_mem), &gminBuffer);
    /*
    ...
    */
    ...
    errNum = clSetKernelArg(kernel3, 0, sizeof(cl_mem), &M4buffer);
    ...
    errNum |= clSetKernelArg(kernel3, 5, sizeof(cl_mem), &cost4Buffer);
    errNum |= clSetKernelArg(kernel3, 6, sizeof(cl_mem), &gminBuffer);
    /*
    errNum |= clSetKernelArg(kernel2, 11, sizeof(cl_mem), &input_image);
    errNum |= clSetKernelArg(kernel2, 12, sizeof(cl_sampler), &sampler);
    */
    ...
    const size_t globalWorkSize2[2] = { N/4 , 8 };
    const size_t localWorkSize2[2] = { N >> 8, 1 };
    ...
    while ( !vazio ( Mask, n ) )
    {
        ...
        */
        ...
        errNum = clEnqueueReadBuffer(fila, costBuffer, CL_FALSE, 0, 
                sizeof(cl_int) * n, cost->val, 0, NULL, &releituraFeita);
        ...
        checkErr(errNum, "clEnqueueNDRangeKernel 2");
        }
        ...
        checkErr(errNum, "Error Profiling Kernel 2");
        run_time_k2 += (double)(ev_end_time - ev_start_time)/1e6; // in msec

        errNum = clEnqueueReadBuffer(fila, gminBuffer, CL_FALSE, 0, 
                sizeof(cl_int) * ( N>>8 ), gmin, 0, NULL, &releituraFeita);
        checkErr(errNum, "Reading gmin");
        clWaitForEvents(1, &releituraFeita);
