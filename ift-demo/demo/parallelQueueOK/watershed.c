...
// This describes the format of the image data.
    ...
    */
    ...
    errNum = clSetKernelArg(kernel3, 0, sizeof(cl_mem), &IBuffer);
    errNum |= clSetKernelArg(kernel3, 1, sizeof(cl_mem), &IvalBuffer);
    errNum |= clSetKernelArg(kernel3, 2, sizeof(cl_mem), &Adxbuffer);
    errNum |= clSetKernelArg(kernel3, 3, sizeof(cl_mem), &Adybuffer);
    errNum |= clSetKernelArg(kernel3, 4, sizeof(cl_mem), &Mbuffer);
    errNum |= clSetKernelArg(kernel3, 5, sizeof(cl_mem), &costBuffer);
    errNum |= clSetKernelArg(kernel3, 6, sizeof(cl_mem), &predBuffer);
    errNum |= clSetKernelArg(kernel3, 7, sizeof(cl_mem), &imgValBuffer);
    //errNum |= clSetKernelArg(kernel3, 5, sizeof(cl_mem), &CPredbuffer);
    errNum |= clSetKernelArg(kernel3, 8, sizeof(cl_mem), &SEMbuffer);
    errNum |= clSetKernelArg(kernel3, 9, sizeof(cl_mem), &extraBuffer);
    errNum |= clSetKernelArg(kernel3, 10, sizeof(cl_mem), &labelBuffer);
    errNum |= clSetKernelArg(kernel3, 11, sizeof(cl_mem), &cacheBuffer);
    /*
    errNum |= clSetKernelArg(kernel2, 11, sizeof(cl_mem), &input_image);
    errNum |= clSetKernelArg(kernel2, 12, sizeof(cl_sampler), &sampler);
    */
